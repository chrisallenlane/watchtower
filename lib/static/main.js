        ...
        data_store = {
            ...
            save: function(){
                $('div.point_of_interest').each(function(index, element){
                    this.points[index] = $(element).attr('class');
                });
                console.log(this.points);
                localStorage.<%= watchtower_storage %> = window.JSON.stringify(this.points);
            },
            
            /*
            load: function(){
                this.points = jQuery.parseJSON(localStorage.<%= watchtower_storage %>);
                $('div.point_of_interest').each(function(index, element){
                    // do this.save(), but in reverse
                    if(this.points[index] != ''){
                        $(element).attr('class', this.points[index]);
                    }
                });
