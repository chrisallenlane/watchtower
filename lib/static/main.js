        ...
        data_store  = {
            ...
            save:   function(){
                        $('div.point_of_interest').each(function(index, element){
                            data_store.points[index] = $(element).attr('class');
                        });
                        localStorage.data_store_nav    = $('div.column_nav').html();
                        localStorage.data_store_points = window.JSON.stringify(data_store.points);
                    },
            load: function(){
                if(localStorage.data_store != null){
                    // set the state on all of the data points
                    this.points = jQuery.parseJSON(localStorage.data_store_points);
                    $('div.point_of_interest').each(function(index, element){
                        // do this.save(), but in reverse
                        if(data_store.points[index] != ''){
                            $(element).attr('class', data_store.points[index]);
                        }
                    });
                    
                    //re-load the saved navigation
                    this.nav = localStorage.data_store_nav
                    $('div.column_nav').html(this.nav);
                    
                    // set each Poi's markers appropriately
                    $('div.point_of_interest.poi_ok span.poi_mark_content a.poi_ok').addClass('selected');
                    $('div.point_of_interest.poi_bad span.poi_mark_content a.poi_bad').addClass('selected');
                    // hide hidden PoIs
